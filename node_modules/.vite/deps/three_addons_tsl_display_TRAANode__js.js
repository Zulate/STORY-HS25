...
var TRAANode = class extends TempNode {
  ...
  constructor(beautyNode, depthNode, velocityNode, camera) {
    ...
    this._cameraWorldMatrix = uniform2(new Matrix4());
    this._cameraProjectionMatrixInverse = uniform2(new Matrix4());
    this._previousCameraWorldMatrix = uniform2(new Matrix4());
    this._previousCameraProjectionMatrixInverse = uniform2(new Matrix4());
    ...
    this._textureNode = passTexture2(this, this._resolveRenderTarget.texture);
    this._originalProjectionMatrix = new Matrix4();
    this._previousDepthNode = texture2(new DepthTexture(1, 1));
  ...
  setViewOffset(width, height) {
    ...
    velocity2.setProjectionMatrix(this._originalProjectionMatrix);
    const viewOffset = {
      fullWidth: width,
      fullHeight: height,
      offsetX: 0,
  ...
  clearViewOffset() {
    ...
    velocity2.setProjectionMatrix(null);
    this._jitterIndex++;
  ...
  updateBefore(frame) {
    ...
    renderer.setRenderTarget(this._resolveRenderTarget);
    _quadMesh2.material = this._resolveMaterial;
    _quadMesh2.name = "TRAA";
  ...
  setup(builder) {
    ...
    if (postProcessing) {
      ...
      postProcessing.context.onBeforePostProcessing = () => {
        const size = builder.renderer.getDrawingBufferSize(_size2);
    ...
    const historyTexture = texture2(this._historyRenderTarget.texture);
    const sampleTexture = this.beautyNode;
    const depthTexture = this.depthNode;
    const velocityTexture = this.velocityNode;
    const resolve = Fn2(() => {
      const uvNode = uv2();
      const minColor = vec42(1e4).toVar();
      const maxColor = vec42(-1e4).toVar();
      const closestDepth = float2(1).toVar();
      const farthestDepth = float2(0).toVar();
      const closestDepthPixelPosition = vec22(0).toVar();
      Loop2({ start: int2(-1), end: int2(1), type: "int", condition: "<=", name: "x" }, ({ x }) => {
        Loop2({ start: int2(-1), end: int2(1), type: "int", condition: "<=", name: "y" }, ({ y }) => {
          const uvNeighbor = uvNode.add(vec22(float2(x), float2(y)).mul(this._invSize)).toVar();
          const colorNeighbor = max2(vec42(0), sampleTexture.sample(uvNeighbor)).toVar();
          minColor.assign(min2(minColor, colorNeighbor));
          maxColor.assign(max2(maxColor, colorNeighbor));
          const currentDepth2 = depthTexture.sample(uvNeighbor).r.toVar();
          If2(currentDepth2.lessThan(closestDepth), () => {
            closestDepth.assign(currentDepth2);
            closestDepthPixelPosition.assign(uvNeighbor);
          });
          If2(currentDepth2.greaterThan(farthestDepth), () => {
            farthestDepth.assign(currentDepth2);
          });
        });
      });
      const offset = velocityTexture.sample(closestDepthPixelPosition).xy.mul(vec22(0.5, -0.5));
      const currentColor = sampleTexture.sample(uvNode);
      const historyColor = historyTexture.sample(uvNode.sub(offset));
      const clampedHistoryColor = clamp2(historyColor, minColor, maxColor);
      const currentDepth = depthTexture.sample(uvNode).r;
      const currentViewPosition = getViewPosition2(uvNode, currentDepth, this._cameraProjectionMatrixInverse);
      const currentWorldPosition = this._cameraWorldMatrix.mul(vec42(currentViewPosition, 1)).xyz;
      const historyUV = uvNode.sub(offset);
      const previousDepth = this._previousDepthNode.sample(historyUV).r;
      const previousViewPosition = getViewPosition2(historyUV, previousDepth, this._previousCameraProjectionMatrixInverse);
      const previousWorldPosition = this._previousCameraWorldMatrix.mul(vec42(previousViewPosition, 1)).xyz;
      const worldPositionDifference = length2(currentWorldPosition.sub(previousWorldPosition)).toVar();
      worldPositionDifference.assign(min2(max2(worldPositionDifference.sub(1), 0), 1));
      const velocityMagnitude = length2(offset).toConst();
      const motionFactor = max2(worldPositionDifference.mul(0.5), velocityMagnitude.mul(10)).toVar();
      motionFactor.assign(min2(motionFactor, 1));
      const currentWeight = float2(0.05).add(motionFactor.mul(0.25)).toVar();
      ...
      If2(strongDisocclusion, () => {
        currentWeight.assign(1);
        historyWeight.assign(0);
      });
      const compressedCurrent = currentColor.mul(float2(1).div(max2(currentColor.r, currentColor.g, currentColor.b).add(1)));
      const compressedHistory = clampedHistoryColor.mul(float2(1).div(max2(clampedHistoryColor.r, clampedHistoryColor.g, clampedHistoryColor.b).add(1)));
      const luminanceCurrent = luminance2(compressedCurrent.rgb);
      const luminanceHistory = luminance2(compressedHistory.rgb);
      currentWeight.mulAssign(float2(1).div(luminanceCurrent.add(1)));
      historyWeight.mulAssign(float2(1).div(luminanceHistory.add(1)));
      const smoothedOutput = add2(currentColor.mul(currentWeight), clampedHistoryColor.mul(historyWeight)).div(max2(currentWeight.add(historyWeight), 1e-5)).toVar();
...
var traa = (beautyNode, depthNode, velocityNode, camera) => nodeObject2(new TRAANode(convertToTexture2(beautyNode), depthNode, velocityNode, camera));
export {
